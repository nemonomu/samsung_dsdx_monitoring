...
- [x] 후속 요청 반영: 리뷰 수 0·본문 0으로 감소한 경우 이상치 대신 확인 필요로 표시한다. 비교일 리뷰 수가 0인 감소도 확인 필요다. 두 날짜 모두 본문 0이면 감소가 아니므로 이 규칙에서는 검출하지 않는다. 기존 별점·카운트 불일치 규칙은 별도로 유지한다.
...
- [x] 리뷰 수 0·본문 0으로 감소하면 별점 수가 남아 있어도 본문 감소는 확인 필요로 표시한다. 그 밖의 카운트 불일치는 감소 허용 예외가 아니다. NULL·빈값·잘못된 숫자는 0으로 취급하지 않는다.
...
## 후속 수정: 0건 확인 필요 및 카운트 동시 표시

- [x] 본문 감소의 0건 예외를 확인 필요로 분류하고 상세·요약·정상 확인에 연결한다. 과거 비교 행도 두 카운트를 그대로 보여준다.
- [x] 리뷰 수 일치 대상은 다른 크로스필드 상세에서도 두 카운트를 나란히 기본 표시한다: SEA Bestbuy TV/REF/LDY, Lowes REF/LDY, SEG Mediamarkt·OTTO, TSE Homepro, SEM Liverpool.
- [x] 관련 Python 테스트 33개 및 JavaScript 테스트 파일 2개 통과.
- [ ] 이번 변경 운영 반영: 코드 배포, collectstatic, gunicorn 재시작. 앞선 규칙 SQL 등록을 완료했다면 SQL을 다시 실행할 필요는 없다.
