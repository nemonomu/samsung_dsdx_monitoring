...
function testTseCorrectionSqlEscapesLiteralsAndHtml() {
    ...
    assert.ok(query.includes("crawl_datetime >= '2026-08-08'"));
    assert.ok(query.includes("crawl_datetime < '2026-08-11'"));
    assert.ok(query.includes("account_name = 'Home''pro'"));
    assert.ok(query.includes("'TV''1 <script>'"));
    assert.ok(query.includes("country = 'TSE'"));
    assert.ok(query.includes('ORDER BY item, crawl_datetime;'));
    assert.ok(!/LOWER|UPPER|TRIM|CAST|WITH latest|IS NOT DISTINCT/.test(query));
    ...
    assert.ok(singleDayQuery.includes("crawl_datetime >= '2026-08-10'"));
