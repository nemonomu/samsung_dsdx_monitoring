...
class HomeDepotLayer2Tests(unittest.TestCase):
        ...
        for product in ('ref', 'ldy'):
            for field in policy.homedepot_format_columns(product):
                rules.append(dict(table_name=product + '_retail_com', column_name=field,
                                  account_name='HomeDepot', check_type='range_float' if field == 'star_rating' else 'regex',
                                  pattern=None if field == 'star_rating' else patterns['SEA_HOMEDEPOT_' + fields[field]],
                                  rule_value='0~5' if field == 'star_rating' else None, error_message='invalid'))
    ...
    def test_format_fetch_has_seven_fields_and_no_main_anchor_for_homedepot(self):
        ...
        self.assertEqual(7, len(self.format._get_sea_format_fields('ldy', 'HomeDepot')))
