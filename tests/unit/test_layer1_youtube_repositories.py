...
class YouTubeRepositoryTests(unittest.TestCase):
    ...
    def test_expected_excludes_legacy_group(self):
        ...
        self.assertIn('COUNT(DISTINCT region_code)', sql)
        self.assertNotIn('COUNT(DISTINCT collection_country)', sql)
        self.assertNotIn('COUNT(DISTINCT target_channel_country)', sql)
