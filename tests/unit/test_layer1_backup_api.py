...
class Layer1BackupApiTests(unittest.TestCase):
    def test_get_returns_all_pending_backup_counts(self):
        ...
        self.assertEqual(response['sem_tv_count'], 10)
        self.assertEqual(response['sem_ref_count'], 11)
        self.assertEqual(response['sem_ldy_count'], 12)
        self.assertEqual(response['seg_tv_count'], 13)
        self.assertEqual(response['seg_ref_count'], 14)
        self.assertEqual(response['seg_ldy_count'], 15)
    ...
    def test_post_returns_actual_integrated_backup_counts(self):
        ...
        self.assertIn('SEM TV: 10건', response['message'])
        self.assertIn('SEM REF: 11건', response['message'])
        self.assertIn('SEM LDY: 12건', response['message'])
        self.assertIn('SEG TV: 13건', response['message'])
        self.assertIn('SEG REF: 14건', response['message'])
        self.assertIn('SEG LDY: 15건', response['message'])
