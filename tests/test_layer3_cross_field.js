...
assert(source.includes("review_body_count: { label: '리뷰본문 수'"));
assert(source.includes("count_of_reviews: { label: 'count_of_reviews'"));
assert(source.includes("detailed_review_content: { label: 'detailed_review_content'"));
assert(source.includes("crawl_datetime: { label: 'crawl_datetime', width: 190"));
assert(source.includes("crawl_strdatetime: { label: 'crawl_strdatetime', width: 190"));
assert(!source.includes("max_review_number: { label:"));
assert(!source.includes("review_body_criterion: { label:"));
assert(source.includes("'cf-target-date-row' : 'cf-history-date-row'"));
assert(source.includes('수정 대상'));
assert(source.includes('비교 이력'));
assert(source.includes('더블클릭해 수정'));
...
assert(source.includes("/^(SEA_|SEDA_|SIEL_|SEG_|SEM_|TSE_)/.test(productLineDisplay)"));
