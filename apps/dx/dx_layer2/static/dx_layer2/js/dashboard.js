...
const LAYER2_NULL_TABLE_GROUPS = [
    ...
    },
    {
        key: 'sem',
        name: 'SEM Retail',
        description: 'SEM TV/REF/LDY NULL 검증',
        tableCodes: ['sem_tv_retail', 'sem_ref_retail', 'sem_ldy_retail']
    },
    {
        key: 'tse',
        name: 'TSE Retail',
...
function renderDXTableDetail(vType, table) {
    ...
    if (vType.type === 'duplicate' && table.retailers) {
        ...
        table.retailers.forEach(retailer => {
            ...
            } else if (isSeaRetail && String(retailer.retailer).toLowerCase() === 'homedepot') {
                detailText = '최신 배치의 Item 중복 및 상품 매핑 충돌';
            } else if (isSeaRetail || isSegRetail || isSedaRetail) {
