    ...
    function renderEmailReport(dailyData, emailData, date) {
        ...
        dailyRows.forEach(function(r, rowIndex) {
            var isGroupStart = rowIndex > 0
                && dailyRows[rowIndex - 1].table_group !== r.table_group;
            var groupDivider = isGroupStart
                ? ' style="border-top:2px solid #1f4e78;"'
                : '';
            html += '<tr>';
        ...
        if (sendBtn) {
            sendBtn.addEventListener('click', function() {
                ...
                if (selectedDate > todayStr) {
                    showToast('내일 이후 검수일은 이메일 발송할 수 없습니다.', 'warning');
                    return;
                }
