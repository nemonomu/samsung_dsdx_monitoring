(function() {
    ...
    function category(cat, checkIdx, catIdx) {
        ...
        return '<div class="sentiment-category-item">' +
            '<div class="sentiment-category-header" onclick="toggleSedaCategory(this,' + checkIdx + ',' + catIdx + ')">' +
                ...
                '<div class="sentiment-category-stats">' + L1.retailQuery.button('SEDA', cat, checkIdx, catIdx) +
                '<span class="sentiment-category-count">' + countLabel + '</span>' +
                getStatusBadge(cat.status) + '</div></div>' +
            '<div class="sentiment-two-column retail-single-column" id="seda-cat-' + checkIdx + '-' + catIdx + '">' +
    ...
    function render(check, checkIdx) {
        return '<div class="check-item"><div class="check-main retail-check-main" onclick="L1.retailStatus.toggle(this,' + checkIdx + ')">' +
            ...
            renderCountryFlagLabel(check.name || 'SEDA Retail') + '</div><div class="check-description">' +
            esc(check.description || '') + '</div></div>' +
            L1.retailStatus.render(check, checkIdx, 'seda_retail') +
            ...
            '<div class="time-slot-info"><span class="time-slot-name">데이터 기준</span>' +
            '<span class="time-slot-time"><span class="utc">' +
            esc(check.collection_window || '검수일 D-1 데이터') +
            '</span></span></div></div></div><div class="sentiment-categories">' +
            (check.categories || []).map(function(cat, catIdx) {
                return category(cat, checkIdx, catIdx);
            }).join('') + '</div></div></div>';
    }

    window.toggleSedaCategory = function(element, checkIdx, catIdx) {
        var container = document.getElementById('seda-cat-' + checkIdx + '-' + catIdx);
    ...
    L1.renderers.seda_retail = render;
})();
