...
def get_collection_status(target_date, now=None):
    ...
    for source in SOURCES:
        ...
        row = {'retailer': retailer, 'product': product,
               ...
               'count': 0, 'last_collected_at': None, 'batch_id': None,
        ...
        if enabled:
            try:
                with dx_connection() as (_conn, cursor):
                    count, last_at, batch = fetch_collection(cursor, source, target_date)
                row.update(count=int(count or 0), last_collected_at=last_at, batch_id=batch)
                if row['count']:
                    row['status'] = 'received'
            except Exception:
                row.update(status='error', count=None)
        else:
