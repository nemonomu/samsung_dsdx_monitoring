-- Run manually in PostgreSQL/DBeaver. Only updates SEA LDY / Lowes rules.
-- Source data and REF rules are unchanged.
...
BEGIN
    FOR rule_seed IN
        SELECT * FROM (VALUES
            ('discount_type', 'SEA_APPLIANCE_LOWES_LDY_DISCOUNT',
             ...
             'Lowes LDY discount phrases including Buy More and Buy N And Get N',
             'discount_type은 허용된 Lowes 할인 문구여야 하며 금액과 수량은 양의 정수, 할인율은 1~100 정수여야 합니다.'),
            ('pick_up_availability', 'SEA_APPLIANCE_LOWES_LDY_PICKUP',
             ...
             'Lowes LDY pickup date with optional (Est.), Pickup Ready Today or Pickup Ready Tomorrow',
    ...
    LOOP
        ...
        WHERE table_name = 'ldy_retail_com' AND account_name = 'Lowes'
        ...
        IF target_template_id IS NULL THEN
            INSERT INTO public.monitoring_format_templates (
                ...
                'lowes_ldy_formats', NOW(), 'lowes_ldy_formats', NOW()
        ...
        ELSE
            ...
            SET pattern = rule_seed.pattern, check_type = 'regex', is_active = TRUE,
                ...
                updated_id = 'lowes_ldy_formats', updated_at = NOW()
...
WHERE r.table_name = 'ldy_retail_com' AND r.account_name = 'Lowes'
